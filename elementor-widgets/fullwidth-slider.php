...
class Elementor_Fullwidth_Slider_Widget extends \Elementor\Widget_Base {
    ...
    private function get_travel_categories() {
        $categories = get_terms(array('taxonomy' => 'tpm_category', 'hide_empty' => true));
        $options = array();
    ...
    private function get_wedding_categories() {
        $categories = get_terms(array('taxonomy' => 'wedding_category', 'hide_empty' => true));
        $options = array();
        if (!is_wp_error($categories) && !empty($categories)) {
            foreach ($categories as $cat) {
                $options[$cat->term_id] = $cat->name . ' (' . $cat->count . ')';
    ...
    private function get_travel_regions() {
        $regions = get_terms(array('taxonomy' => 'tpm_region', 'hide_empty' => true));
        $options = array();
    ...
    private function get_wedding_regions() {
        $regions = get_terms(array('taxonomy' => 'wedding_region', 'hide_empty' => true));
        $options = array();
        if (!is_wp_error($regions) && !empty($regions)) {
            foreach ($regions as $region) {
                $options[$region->term_id] = $region->name . ' (' . $region->count . ')';
    ...
    protected function render() {
        ...
        if (is_wp_error($query)) {
            error_log('WEDYARA ERROR: Slider query failed - ' . $query->get_error_message());
        }
