...
// Handle CSV download BEFORE admin page loads
add_action('admin_init', 'stp_handle_csv_download_early');

function stp_handle_csv_download_early() {
    // Check if this is a CSV download request
    if (isset($_GET['page']) && $_GET['page'] === 'stp-csv-import-fixed' &&
        isset($_GET['action']) && $_GET['action'] === 'download_sample' &&
        isset($_GET['sample_nonce']) && wp_verify_nonce($_GET['sample_nonce'], 'download_sample_csv')) {
...
function stp_download_sample_csv_fixed() {
    // Clean all output buffers to prevent HTML from being included
    while (ob_get_level()) {
        ob_end_clean();
    }
