<?php
/**
 * CSV Import Functionality for Travel Packages
 * Supports HTML formatting from Word documents
 */

// Add admin menu for CSV import
add_action('admin_menu', 'stp_csv_import_menu', 20);

function stp_csv_import_menu() {
    add_submenu_page(
...
function stp_csv_import_page() {
    ...
    <div class="wrap stp-import-wrapper">
        <h1 class="stp-import-title">
            ...
            Import Travel Packages from CSV
        </h1>
        ...
        <div class="stp-import-grid">
            ...
            <div class="stp-import-card stp-upload-card">
                ...
                <form method="post" enctype="multipart/form-data" class="stp-upload-form">
                    <?php wp_nonce_field('stp_csv_import_nonce', 'stp_csv_import_nonce'); ?>
                ...
                <?php
                // Handle import
                if (isset($_POST['import_csv']) && check_admin_referer('stp_csv_import_nonce', 'stp_csv_import_nonce')) {
...
function stp_process_csv_import() {
    ...
    while (($data = fgetcsv($handle)) !== false) {
        ...
        if ($skip_duplicates) {
            $existing = get_page_by_title($row['title'], OBJECT, 'travel_package');
            if ($existing) {
                $skipped++;
                continue;
            }
    ...
    <div class="stp-import-result <?php echo $errors > 0 ? 'error' : ''; ?>">
        ...
        <p style="margin-top: 20px;">
            <a href="<?php echo admin_url('edit.php?post_type=travel_package'); ?>" class="button button-primary">
...
function stp_import_handle_taxonomies($post_id, $row, $create_categories) {
    $taxonomies = array(
        'categories' => 'tpm_category',
        'package_types' => 'tpm_package_type',
        'activity_types' => 'tpm_activity_type',
        'amenities' => 'tpm_amenities',
        'difficulty_level' => 'tpm_difficulty_level',
        'season' => 'tpm_season',
